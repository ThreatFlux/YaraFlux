...
from yaraflux_mcp_server.models import YaraRuleMetadata, YaraMatch, YaraScanResult
...
class YaraService:
    ...
    def match_file(
        ...
        try:
            ...
            if file_size > settings.YARA_MAX_FILE_SIZE:
                ...
                raise YaraError(
                    f"File too large: {file_size} bytes (max {settings.YARA_MAX_FILE_SIZE} bytes)"
                )
    ...
    def match_data(
        ...
        if file_size > settings.YARA_MAX_FILE_SIZE:
            ...
            raise YaraError(
                f"Data too large: {file_size} bytes (max {settings.YARA_MAX_FILE_SIZE} bytes)"
            )
