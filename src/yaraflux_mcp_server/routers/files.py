...
from yaraflux_mcp_server.models import (
    User, ErrorResponse, 
    FileInfo, FileUploadRequest, FileUploadResponse, FileListResponse,
    FileStringsRequest, FileStringsResponse,
    FileHexRequest, FileHexResponse, FileDeleteResponse
)
from yaraflux_mcp_server.storage import get_storage_client, StorageError
...
async def delete_file(
    file_id: UUID,
    current_user: User = Depends(validate_admin)  # Only admins can delete
):
    ...
    try:
        ...
        if result:
            ...
            return FileDeleteResponse(
                file_id=file_id,
                success=False,
                message=f"File could not be deleted"
            )
...
async def extract_strings(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Error extracting strings: {str(e)}"
        )
...
async def get_hex_view(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Error getting hex view: {str(e)}"
        )
