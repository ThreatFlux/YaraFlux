        ...
        if not settings.USE_MINIO:
            try:
                ...
                info["usage"] = {
                    ...
                    "total": {
                        "file_count": rules_usage["file_count"]
                        + samples_usage["file_count"]
                        + results_usage["file_count"],
                        "size_bytes": rules_usage["size_bytes"]
                        + samples_usage["size_bytes"]
                        + results_usage["size_bytes"],
                        "size_human": _bytes_to_human(
                            rules_usage["size_bytes"] + samples_usage["size_bytes"] + results_usage["size_bytes"]
                        ),
                    },
                }
    ...
    try:
        ...
        if storage_type == "results" or storage_type == "all":
            try:
                ...
                return {"success": False, "message": f"Error cleaning results: {str(e)}"}
        ...
        if storage_type == "samples" or storage_type == "all":
            try:
                ...
                return {"success": False, "message": f"Error cleaning samples: {str(e)}"}
...
def _get_directory_usage(directory: Path) -> Dict[str, Any]:
    ...
    return {"file_count": file_count, "size_bytes": total_size, "size_human": _bytes_to_human(total_size)}
