...
from typing import BinaryIO, Dict, List, Optional, Union, Any, Tuple, TYPE_CHECKING
...
class LocalStorageClient(StorageClient):
    ...
    def __init__(self):
        ...
        logger.info(f"Initialized local storage: rules={self.rules_dir}, "
...
class MinioStorageClient(StorageClient):
    ...
    def __init__(self):
        ...
        logger.info(f"Initialized MinIO storage: endpoint={settings.MINIO_ENDPOINT}, "
    ...
    def delete_rule(self, rule_name: str, source: str = "custom") -> bool:
        ...
        try:
            ...
            logger.debug(f"Deleted rule {rule_name} from MinIO bucket {self.rules_bucket}/{object_key}")
            return True
        except S3Error as e:
            ...
            for obj in objects:
                ...
                if len(parts) >= 2:
                    ...
                    rules.append({
                        "name": rule_name,
                        "source": obj_source,
                        "created": obj.last_modified.isoformat() if hasattr(obj, 'last_modified') else None,
    ...
    def save_sample(self, filename: str, content: Union[bytes, BinaryIO]) -> Tuple[str, str]:
        ...
        try:
            ...
            logger.debug(f"Saved sample {filename} to MinIO bucket {self.samples_bucket}/{object_key} (hash: {file_hash})")
    ...
    def save_result(self, result_id: str, content: Dict[str, Any]) -> str:
        ...
        try:
            ...
            logger.debug(f"Saved result {result_id} to MinIO bucket {self.results_bucket}/{object_key}")
