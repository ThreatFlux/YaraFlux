...
async def test_lifespan_normal():
    ...
    with patch("yaraflux_mcp_server.app.ensure_directories_exist") as mock_ensure_dirs, \
...
def test_create_app():
    ...
    with patch("yaraflux_mcp_server.app.FastAPI") as mock_fastapi, \
...
def test_mcp_initialization():
    ...
    with patch("yaraflux_mcp_server.app.FastAPI") as mock_fastapi, \
        ...
        with patch.dict('sys.modules', {
            'yaraflux_mcp_server.claude_mcp': mock_claude_mcp,
            'yaraflux_mcp_server.mcp_tools': MagicMock()
        }):
            # Call the function
            create_app()
...
def test_main_entrypoint():
    ...
    with patch("uvicorn.run") as mock_run, \
        ...
        if hasattr(uvicorn, "run"):
            ...
            uvicorn.run("yaraflux_mcp_server.app:app", host=mock_settings.HOST, port=mock_settings.PORT, reload=mock_settings.DEBUG)
